...
protected:
  ...
  HRESULT InitializeInternal(LPCWSTR dllName, LPCWSTR dll2Name, LPCSTR fnName) {
    if (m_dll != nullptr) return S_OK;
    m_dll = LoadLibraryW(dllName);
    m_dll2 = LoadLibraryW(dll2Name);
...
public:
  ...
  HRESULT Initialize() {
    return InitializeInternal(L"dxcompiler.dll", L"dxil.dll", "DxcCreateInstance");
  }

  HRESULT InitializeForDll(_In_z_ const wchar_t* dll, _In_z_ const wchar_t* dll2, _In_z_ const char* entryPoint) {
      return InitializeInternal(dll, dll2, entryPoint);
  }
