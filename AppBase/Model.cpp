...
Mesh::Mesh(std::vector<Vertex> vertices, std::vector<std::uint16_t> indices, size_t MatID, XMFLOAT4X4 transformation, std::string alphaMode, RenderDevice* Device) :
	m_Vertices(vertices),
	m_Indices(indices),
	m_MatID(MatID), 
...
void Model::LoadMaterial(aiMaterial* mat, bool HasUV)
{
	std::string tmpstr = mat->GetName().C_Str();
